...
Online Karakterleri Görüntülüyorsnuuz.
</div>
...
<tbody>
<?php
$query = $odb->prepare("SELECT player.*, account.login FROM player.player LEFT JOIN account.account ON account.id = player.account_id 
 WHERE DATE_SUB(NOW(), INTERVAL ? MINUTE) < player.last_play ORDER BY player.level DESC;");
 $query->execute(array(60));
$i = 0;

echo '<code>Toplam '.$query->rowCount().' Online Karakter Var</code>';
