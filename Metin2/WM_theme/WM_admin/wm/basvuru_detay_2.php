...
foreach($basvuranlar as $basvuran => $lonca)
{
	
$i++;

$lonca_uyeleri = $odb->prepare("SELECT player.name AS baskan FROM player.guild_member 
INNER JOIN player.guild ON guild.id = guild_member.guild_id LEFT JOIN player.player ON player.id = guild.master WHERE guild.name = ? GROUP BY guild_member.pid ");
$lonca_uyeleri->execute(array($lonca));

$fetch2 = $lonca_uyeleri->fetch(PDO::FETCH_ASSOC);
	
?>
<tr id="basvuru-<?=$i;?>">
<td WIDTH=15><?=$i;?></td>
<td><a href="index.php?sayfa=kullanicilar&login=<?=$basvuran;?>" target="_blank"><?=$basvuran;?></a></td>
<td><a href="index.php?sayfa=lonca&name=<?=$lonca;?>" target="_blank"><?=$lonca;?></a></td>
<td><?=$lonca_uyeleri->rowCount();?> Üye</td>
<td><a href="index.php?sayfa=karakterler&name=<?=$fetch2["baskan"];?>" target="_blank"><?=$fetch2["baskan"];?></a></td>
<td>
...
foreach($onaylananlar as $basvuran => $lonca)
{
...
foreach($red_edilenler as $basvuran => $lonca)
{
