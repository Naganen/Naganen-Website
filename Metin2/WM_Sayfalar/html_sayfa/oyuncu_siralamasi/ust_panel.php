...
<table border="0" align="center" width="100%">
		...
		<option value="oyuncu-siralamasi?isim=<?=$isim;?>&karakter=" <?=($karakter == "") ? "selected" : "";?>>[Tüm Sınıflar]</option>
		<option value="oyuncu-siralamasi?isim=<?=$isim;?>&karakter=savasci" <?=($karakter == "savasci") ? "selected" : "";?> >Savaşçı</option>
		<option value="oyuncu-siralamasi?isim=<?=$isim;?>&karakter=ninja" <?=($karakter == "ninja") ? "selected" : "";?> >Ninja</option>
		<option value="oyuncu-siralamasi?isim=<?=$isim;?>&karakter=sura" <?=($karakter == "sura") ? "selected" : "";?> >Sura</option>
		<option value="oyuncu-siralamasi?isim=<?=$isim;?>&karakter=saman" <?=($karakter == "saman") ? "selected" : "";?> >Şaman</option>
		</select>
</td>
		<td><input name="arancak" <?=($isim == "") ? 'placeholder="Arancak Karakter Adı"' : 'value="'.$isim.'"';?> type="text" /></td>
		<td><input type="submit" value="Ara" onclick="oyuncu_ara()" /></td>
	</tr>
