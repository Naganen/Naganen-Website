...
class kaydol {
    ...
    public function orta( ) {
        global $ayar, $odb, $WMkontrol, $vt, $db, $tema, $WMclass;
        if ( !isset( $_SESSION[ $vt->a( "isim" ) . "token" ] ) ) {
            if ( $vt->a( "kayit" ) == 2 ) {
                ...
                if ( @$vt->a( "davet_durum" ) == 1 ) {
                    @$davet_token = $WMkontrol->WM_get( $WMkontrol->WM_tostring( $_GET[ "davet" ] ) );
                    if ( $davet_token != "" ) {
                ...
                if ( $_POST ) {
                    $real_name         = $WMkontrol->WM_post( $WMkontrol->WM_tostring( $_POST[ "real_name" ] ) );
                    $username          = $WMkontrol->WM_post( $WMkontrol->WM_tostring( $_POST[ "username" ] ) );
                    ...
                    $social_id         = $WMkontrol->WM_post( $WMkontrol->WM_tostring( $_POST[ "social_id" ] ) );
                    $phone_number      = $WMkontrol->WM_post( $WMkontrol->WM_tostring( $_POST[ "phone_number" ] ) );
                    $eposta            = $WMkontrol->WM_post( $WMkontrol->WM_tostring( $_POST[ "eposta" ] ) );
                    $kayit_token       = $WMkontrol->WM_post( $WMkontrol->WM_tostring( $_POST[ "kayit_token" ] ) );
                    ...
                    if ( $vt->a( "guvenlik" ) == 1 ) {
                        $guvenlik_soru  = $WMkontrol->WM_post( $WMkontrol->WM_toint( $WMkontrol->WM_char_int( $_POST[ "guvenlik_soru" ] ) ) );
                        $guvenlik_cevap = $WMkontrol->WM_post( $WMkontrol->WM_tostring( $_POST[ "guvenlik_cevap" ] ) );
                    } else {
                        $guvenlik_soru  = 0;
                        $guvenlik_cevap = "";
                    }
                    ...
                    try {
                        if ( !$kayit_token ) {
                        ...
                        } else if ( $WMkontrol->WM_mail( $eposta ) == false ) {
                            ...
                            if ( $kontrol->rowCount() ) {
                                ...
                                if ( $kayit->errorInfo()[2] == false ) {
                                    ...
                                    $tema->hata( "Kayıt olunurken bir hata meydana geldi. Bu hata ile yöneticiler ilgileniyor." );
