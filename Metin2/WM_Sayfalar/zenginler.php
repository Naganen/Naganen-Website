...
class zenginler {
    public function head( ) {
        global $vt;
        if ( file_exists( WM_tema . 'sayfalar/zenginler/header.php' ) ) {
            require_once WM_tema . 'sayfalar/zenginler/header.php';
        } else {
    ...
    public function orta( ) {
        ...
        if ( $vt->a( "breadcumb" ) == 1 ) {
            if ( file_exists( WM_tema . 'sayfalar/zenginler/breadcumb.php' ) ) {
                require_once WM_tema . 'sayfalar/zenginler/breadcumb.php';
            } else {
                require_once Sayfa_html . 'breadcumb.php';
            }
        }
        if ( file_exists( WM_tema . 'sayfalar/zenginler/ust_panel.php' ) ) {
            require_once WM_tema . 'sayfalar/zenginler/ust_panel.php';
        } else {
	...
	WHERE player.name NOT LIKE ? && account.status != ? && (player.name LIKE ?) $ara
	GROUP BY player.id
	ORDER BY player.gold DESC 
	LIMIT $limit, $sayfada" );
            $query->execute( $veriler_toplam );
            if ( file_exists( WM_tema . 'sayfalar/zenginler/siralama.php' ) ) {
                require_once WM_tema . 'sayfalar/zenginler/siralama.php';
            } else {
                require_once Sayfa_html . 'siralama.php';
            }
        } else {
            echo "Online Karakter Bulunamadı";
        }
		}
		else{
